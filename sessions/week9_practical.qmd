...
We're going to use the filtered school data that has been used in previous practicals.
...
Please note - we remove the count variables (e.g. NORB and NORG), as the count variables are affected by the pupil size of each school. We also remove variables that are redundant due to perfect correlation with those retained, such as PNUMENGFL, which is equal to 1-PNUMEAL.
...
print("Are all missing values handled?:", df_school_reduced.isnull().sum().sum() == 0)
```
...
After the standardisation, all variables have zero mean and unit variance (close to 1) so that they are comparable.
...
We can choose the number of PCs using a Scree plot and elbow method, which is based on variance explained of each PC.
...
The Scree plot and Elbow method suggested using k=3 PCs for further analysis.

## Task three: to validate and visualise the selected PCs

How do school look like and distribute in the top two PCs? To visualise them, we can use the following code and two PCs.
...
While the code above is equivalent to the code in the next cell, the code in the next cell is more concise and elegant.
