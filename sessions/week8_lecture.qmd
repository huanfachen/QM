...
`y ~ fixed_effect_x + (random_effect_slope_term_x | random_effect_group_g)`


## The Null Model - Syntax
-   Using Paul Johnson's excellent guide for syntax, the null model in R follows this format:`y ~ 1 + (1 | g)`
-   In the null model, the fixed effects appear outside of the brackets. Here the first `1` outside of the brackets just tells the model to use a fixed global intercept
-   The random effects are in the brackets `(...|...)`  are denoted to the *right* of the vertical bar or pipe symbol `|` inside the brackets. To the *left* we have the sub-model. 
...
$$\texttt{lmer(P8MEA \sim PTFSM6CLA1A + (1 \mid gor\_name/LANAME) + (1 \mid OFSTEDRATING), data = england\_filtered\_clean)}$$
~kitchen sink model!!
