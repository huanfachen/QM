...
```
-   Shrinkage accounts for the fact that some groups have few observations / high variance / unreliable means
-   The means for these groups with few observations are 'shrunk' towards the overall mean
...
-   Using [Paul Johnson's](https://paulrjohnson.net/blog/2022-11-01-multilevel-model-r-cheatsheet/) excellent guide for syntax, the null model in R follows this format:
`y ~ 1 + (1 | g)`
...
-   `y ~ x + (1 | g)`
-   In this model, each group in the Random Effect part of the model has it's own intercept and fixed slope still - `(1 | g)`
