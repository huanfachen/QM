...
## Interpreting the Final Model {.smaller}

Random Effects:

| Group                         | Std. Dev |
|-------------------------------|----------|
| SD (Intercept gor_name)       | 0.016    |
| SD (Intercept OFSTEDRATING)   | 0.057    |
| SD (Observations)             | 0.071    |
| SD (Intercept LANAMEgor_name) | 0.026    |

-   Random effects represent variation across groups
-   SD tells us the typical size of deviation from the overall mean for a group.
-   A typical OFSTED group differs from the overall mean by about ±1 SD (≈ 0.057 on the log scale)
...
| Predictor                | Scenario   | Log shift           | \% change | Raw points |
|---------------|---------------|---------------|---------------|---------------|
| **Absence (PERCTOT)**    | 40% → 30%  | −0.2238 × ln(30/40) | +6.7%     | +3.33      |
|                          | 20% → 10%  | −0.2238 × ln(10/20) | +16.8%    | +8.39      |
| **Disadvantage (FSM6)**  | 40% → 30%  | −0.0754 × ln(30/40) | +2.2%     | +1.10      |
|                          | 20% → 10%  | −0.0754 × ln(10/20) | +5.4%     | +2.68      |
| **Prior low attainment** | 40 → 30 pp | −0.00686 × (−10)    | +7.1%     | +3.55      |
|                          | 20 → 10 pp | same                | +7.1%     | +3.55      |
