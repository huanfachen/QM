...
`y ~ 1 + (1 | g)`
-   **Fixed effects outside of the brackets**. The first `1` just tells the model to use a fixed global intercept
-   **Random Effects are inside the brackets** `(...|...)`
    -   To the *left* of the pipe symbol `|` we have a `1` as all slopes are fixed
...
```
-   Global Average Progress 8 is -0.26
-   Progress 8 varies by 0.31 points on average ***between*** Ofsted categories
-   Variation between schools ***within*** categories is smaller: 0.17 points
...
-   High ICC means clustering is important. Ignoring it (e.g., using OLS) would underestimate uncertainty - justifies LME
